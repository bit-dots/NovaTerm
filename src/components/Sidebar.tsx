...
export default function Sidebar({ activeTab }: SidebarProps) {
  ...
  return (
    <div className="flex w-60 flex-col border-r border-border bg-panel-alt">
      {activeTab === "serial" && (
        <>
          <div className="px-4 py-3 text-sm font-semibold uppercase tracking-wider text-text-secondary">
            {t("serial.refresh")}
          </div>
          <div className="flex-1 px-4 py-2 text-base text-text-muted">{t("serial.no_ports")}</div>
        </>
      )}
